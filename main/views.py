...
def index(request):
    ...
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.all().filter(activeForSite=True).filter(activeListings__lte=2))
    regCars = carFilter.qs.filter(listing_type='Regular').order_by('-post_created_at')
    post_obj_db = list(carFilter.qs.values().filter(Q(listing_type='sale') | Q(listing_type='new')).filter(Q(site='Dubicars') | Q(site='facebook')).annotate(int_post_created_at=Cast('post_created_at', IntegerField())).order_by('-int_post_created_at'))


    if user_agent.is_mobile:
        context = {
                'filter':carFilter.form,
                'newAll':list(carFilter.qs.values().filter(Q(listing_type='sale') | Q(listing_type='new')).annotate(int_post_created_at=Cast('post_created_at', IntegerField())).order_by('-int_post_created_at'))[0:50],
                'allFilterQs':regCars[0:50],
            }
    ...
    context = {
            'filter':carFilter.form,
            'dubicarsFilterQs':regCars.filter(Q(site="Dubicars") | Q(site="facebook"))[0:50],
...
def load_more(request):
    ...
    limit = 30
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.all().filter(listing_type='Regular').filter(activeForSite=True).filter(activeListings__lte=2).order_by('-post_created_at'))
    post_obj = list(carFilter.qs.values()[total_item:total_item+limit])
    data = {
        'all':post_obj
    }
...
class SendSignalView(BaseSseView):
    def iterator(self):
        ...
        currTime = int(tm())
        if Car_any.objects.filter(Q(listing_type='Sale') | Q(listing_type='New')).filter(activeForSite=True).filter(activeListings__lte=2).filter(post_created_at__gte=time).filter(post_created_at__lt=currTime).exists():
            print('OK')
...
def load_new(request):
    ...
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.filter(activeForSite=True).filter(activeListings__lte=2).all())
    post_obj_db = list(carFilter.qs.values().filter(Q(listing_type='Sale') | Q(listing_type='New')).filter(Q(site="Dubicars") | Q(site="facebook")).filter(post_created_at__gte=time).filter(post_created_at__lt=curTime).order_by('brand'))
...
def load_new_all(request):
    time = int(request.GET.get('time'))
    curTime = int(request.GET.get('curTime'))
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.filter(activeForSite=True).filter(activeListings__lte=2).all())
    post_obj = list(carFilter.qs.values().filter(Q(listing_type='Sale') | Q(listing_type='New')).filter(post_created_at__gte=time).filter(post_created_at__lt=curTime).order_by('brand'))
    data = {
        'new_all':post_obj
    }
    return JsonResponse(data=data)

def load_more_new_all(request):
    ...
    print(len(data))
    return JsonResponse(data={'data':data[total:total+50], 'isAll':True if len(data) - total - 50 <= 0 else False})
...
def dubicars(request):
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.filter(activeForSite=True).filter(activeListings__lte=2).all())
    regCars = carFilter.qs.filter(listing_type='Regular').order_by('-post_created_at')
    post_obj_db = list(carFilter.qs.values().filter(Q(listing_type='sale') | Q(listing_type='new')).annotate(int_post_created_at=Cast('post_created_at', IntegerField())).order_by('-int_post_created_at'))

    context = {
        'filter':carFilter.form,
        'allFilterQs':regCars[0:50],
        'newAll':post_obj_db[0:50],
    }
...
def load_more_dubicars(request):
    ...
    limit = 30
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.all().filter(listing_type='Regular').filter(activeForSite=True).filter(activeListings__lte=2).order_by('-post_created_at'))
    post_obj = list(carFilter.qs.values().filter(Q(site="Dubicars") | Q(site="facebook"))[total_item:total_item+limit])
    data = {
        'dubicars':post_obj
    }
    return JsonResponse(data=data)
...
def load_new_dubicars(request):
    ...
    curTime = int(request.GET.get('curTime'))
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.filter(activeForSite=True).filter(activeListings__lte=2).all())
    post_obj = list(carFilter.qs.values().filter(Q(listing_type='Sale') | Q(listing_type='New')).filter(Q(site="Dubicars") | Q(site="facebook")).filter(post_created_at__gte=time).filter(post_created_at__lt=curTime).order_by('brand'))
    data = {
...
def load_more_new_dubicars(request):
    total = int(request.GET.get('total_item'))
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.all().filter(Q(site="Dubicars") | Q(site="facebook")).filter(activeForSite=True).filter(activeListings__lte=2))
    data = list(carFilter.qs.values().filter(Q(listing_type='Sale') | Q(listing_type='New')).order_by('-post_created_at'))
...
def dubizzle(request):
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.filter(site="Dubizzle").filter(activeForSite=True).filter(activeListings__lte=2).all())
    regCars = carFilter.qs.filter(listing_type='Regular').order_by('-post_created_at')
...
def load_more_new_dubizzle(request):
    total = int(request.GET.get('total_item'))
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.all().filter(site='Dubizzle').filter(activeForSite=True).filter(activeListings__lte=2))
    data = list(carFilter.qs.values().filter(Q(listing_type='Sale') | Q(listing_type='New')).order_by('-post_created_at'))
    print(len(data))
    return JsonResponse(data={'data':data[total:total+50], 'all':True if len(data) - total - 50 <= 0 else False})
...
def yallamotor(request):
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.filter(site="Yallamotor").filter(activeForSite=True).filter(activeListings__lte=2).all())
    regCars = carFilter.qs.filter(listing_type='Regular').order_by('-post_created_at')
...
def load_more_new_yallamotor(request):
    total = int(request.GET.get('total_item'))
    carFilter = CarFilter(request.GET, queryset=Car_any.objects.all().filter(site='Yallamotor').filter(activeForSite=True).filter(activeListings__lte=2))
    data = list(carFilter.qs.values().filter(Q(listing_type='Sale') | Q(listing_type='New')).order_by('-post_created_at'))
    print(len(data))
    return JsonResponse(data={'data':data[total:total+50], 'all':True if len(data) - total - 50 <= 0 else False})